...
{: .note }
Heya! Siddarth here. This lecture note covers the entire [intro slide deck](https://docs.google.com/presentation/d/1RG2eC0hrYz8_ExI0D-ocvvmDzxgNND8N/edit#slide=id.p1)). If you have feedback on how this is done, please let me know!
...
TODO: Insert image of stack

Usually local variables and function parameters are stored on the stack. Dynamically allocated objects and values are usually stored on the heap. Most languages store global and static variables in the static data area. Ofcourse, you can also have combinations of these, for example, a pointer that is stored on the stack, but whose value lies on the heap.
...
#### Variable binding
As we have seen in Lecture 1 [ss], different languages have different ways of binding variable names with values. For example a C++ variable name directly refers to the storage location holding it's value, while in Python, a variable holds an "object reference", which then in turn points to the actual value. We will cover the major binding approaches in detail later on.
...
ELUDOM
```
But virtually all modern languages have a type system, since it makes programming so much easier.

Question: In a typed language, must every variable have a type? No! In Python for example, variables are not associated with types.
```python
# Python
def foo(q):
  if q:
  ...
  x = 10 # int
```
However, note that a value is **always** associated with a type.
...
- Tuples
- Containers
There can also be data types that don't fall into either category. These can depend on the language.
- Functions
- Generic types
- Boxed types
...
[SS: put 2d grid of strictness vs compile time]

#### Static typing
Static typing is the process of verifying that all operations in a program are consistent with the types of the operands prior to program execution. Types can either be explicitly specified (like in C++) or can be inferred from code (like in Haskell). For inferred types, usually if the type checker cannot assign distinct types to all variables, functions and expressions and verify type safety, then it generates an error.
But if a program type checks, then it means the code is (largely) type safe, and few if any runtime checks need to be performed.
...
- Produces faster code (since we don't have to type check during runtime)
...
You can make any class iterable by just implementing the `__iter__` and `__next__` methods.
```python
...
You can make any class printable (using `print`) by just implementing the `__str__` function
```python
...
If you add the `__eq__` method to any class, you can make it's objects "comparable".
```python
